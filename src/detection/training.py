...
from constants import MIN_MAX_HEIGHT, BBOX_PARAMS, INPUT_RESOLUTION, IMAGES_PATH, ANNOT_PATH, \
    TEST_CENTER_CROP_RESOLUTION
    ...
    def __init__(self, img_path: str, annot_path: str, transform = None):
        ...
        with open(annot_path, 'r') as f:
            ...
            for annot in data_js['annotations']:
                ...
                if w * INPUT_RESOLUTION[0] / img_w > 1 and h * INPUT_RESOLUTION[1] / img_h > 1:
...
def train(train_ratio: float = 0.9, learning_rate: float = 1e-6, epochs_number: int = 50, **kwargs) -> float:
    ...
    Returns:
        maximal export Average Precision
    """
    ...
    train_transform = A.Compose([
        ...
        'min_height' and 'max_height' in kwargs else MIN_MAX_HEIGHT, size=INPUT_RESOLUTION),
    ...
    test_transform = A.Compose([
        A.CenterCrop(*TEST_CENTER_CROP_RESOLUTION),
        A.Resize(*INPUT_RESOLUTION),
        A.Normalize(*(kwargs['Normalize'] if 'Normalize' in kwargs else ((0, 0, 0), (1, 1, 1))), max_pixel_value=255),
    ], bbox_params=A.BboxParams(format='pascal_voc', label_fields=['labels'], **BBOX_PARAMS))
...
def objective(trial) -> float:
    ...
    n = len(os.listdir(r'../../data/results/detection_study/plots'))
...
def conduct_study():
    """Conduct detection_study using optuna"""
